...
def test_confirm_sector_uses_saved_prices_and_rejects_other_assets():
    reset_runtime_mode_controller(DataMode.LIVE)
    store = SQLiteDecisionEventStore(":memory:")
    client = TestClient(create_app(store))
    headers = {"X-Owner-ID": "sector-owner"}
    saved = client.post("/api/v1/copilot/validate-portfolio-ocr", headers=headers, json={
        "owner_id": "sector-owner", "cash_cny": 1000,
        "positions": [{"asset_id": "300750.SZ", "name": "宁德时代", "asset_class": "STOCK",
            "quantity": 100, "price": 310.25, "cost_price": 300,
            "observed_at": "2026-09-14T14:30:00+08:00"}],
    })
    assert saved.status_code == 200
    payload = {"owner_id": "sector-owner", "data_mode": "LIVE", "sectors": {"300750.SZ": "Industrials"}}
    response = client.patch("/api/v1/advisor/portfolio/sectors", headers=headers, json=payload)
    assert response.status_code == 200
    row = response.json()["positions"][0]
    assert row["price"] == 310.25 and row["quantity"] == 100 and row["cost_price"] == 300
    assert row["sector"] == "Industrials"
    assert "user-confirmed sector" in row["price_source"]
    replaced = client.put("/api/v1/advisor/portfolio/current", headers=headers, json={
        "owner_id": "sector-owner", "data_mode": "LIVE", "positions": [row], "cash_cny": 1000,
    })
    assert replaced.status_code == 200
    assert replaced.json()["positions"][0]["sector"] == "Industrials"
    payload["sectors"] = {"600519.SH": "Consumer"}
    assert client.patch("/api/v1/advisor/portfolio/sectors", headers=headers, json=payload).status_code == 422
    payload["sectors"] = {"300750.SZ": "MadeUp"}
    assert client.patch("/api/v1/advisor/portfolio/sectors", headers=headers, json=payload).status_code == 422
    store.close()
