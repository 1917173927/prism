...
| 真实数据 | LIVE 扶摇行情及基金披露，独立能力降级与重新验证 | 已有真实请求与浏览器探测 | 正式问财授权、外部配额与长期 SLA |
...
| P3 进阶 | X6 依赖画布、版本保存、有界 MOCK 执行；会话前提锁定及漂移拒绝 | 页面及版本/超时/漂移回归 | 任意真实投研节点、完整双视图及逐句幻觉识别 |
...
- P3 原计划包括完整工作流双视图与会话防幻觉控制台；现已增量实现固定研究 DAG 编辑和服务端会话前提锁定，完整范围仍按 TODO 分项验收。设计目标见 [系统总体技术架构与方案设计规范](docs/architecture.md)。
...
- 正式问财 SkillHub 凭据、官方接口文档、配额、留存和展示授权没有确认依据。适配器存在不等于正式服务已连通。
- LIVE 行情与基金披露走扶摇服务端接口；失败返回具体安全错误码，不以静态价格冒充成功。MOCK 仍使用静态合成底稿。基金披露是报告期持仓，不是实时基金交易账户。
- 远程 LLM 仅在配置后调用；本机尚未配置真实模型。自然语言画像和语义检索包含明确标注的有限规则降级，真实模型质量没有完成验收。认证模式不把聊天或模型密钥保存到浏览器；开发模式仍保留原本地缓存行为。
- 本地 HTTP Basic 已提供身份到 owner 的绑定，但本机主服务尚未配置用户密码，当前仍为开发模式。它不替代公网身份服务与生产安全网关，本地审计也不具备独立防篡改保证。
- 已确认风险问卷和持仓可跨刷新、重启恢复；历史上下文可按来源检索，但没有跨设备同步或无限长期对话记忆。券商账户同步缺少已选券商、授权账户及只读接口契约。
...
- 工作流目前只开放固定八节点合成研究的依赖与位置；事实黑板检测服务端结构化前提的漂移，不宣称识别所有自然语言幻觉。完整 P3 规划仍有未实现部分。
