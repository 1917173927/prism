...
- Phase 38 已将默认首页收敛为 Copilot 任务中心：支持三层信息架构、三个示例画像、
  ...
  L3 专家审计；自定义画像、持仓输入、owner 隔离的服务端多会话历史、会话切换/重命名/删除和
  响应式布局也已接入。普通追问由服务端读取同一上下文作用域内最近 6 条完整消息，避免跨组合或
  跨前提版本复用旧结论。
...
- 已确认风险问卷、持仓和对话历史可跨刷新、服务重启恢复；对话按 owner 保存在服务端数据库，页面加载最近 50 个会话、单个会话展示最近 200 条消息，支持自动标题、切换、重命名和删除。模型上下文仍只使用最近 6 条同作用域完整消息；该记录不是决策审计事件，也不构成无限长期金融事实记忆。券商账户同步不在当前范围内。
...
| [项目技术方案](docs/submission/competition-technical-solution.md) | 面向评审的需求、功能展示、核心技术、组合案例与效果验证 |
