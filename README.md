...
- Phase 38 已将默认首页收敛为 Copilot 任务中心：支持三层信息架构、三个示例画像、
  ...
  L3 专家审计；自定义画像、持仓输入、owner 隔离的浏览器端多会话历史、会话切换/删除和
  响应式布局也已接入。普通追问仅携带同一上下文作用域内最近 6 条完整消息，避免跨组合或
  跨前提版本复用旧结论。
...
- 已确认风险问卷和持仓可跨刷新、重启恢复；对话历史按 owner 保存在当前浏览器，最多保留 12 个会话、每个会话 40 条消息，支持刷新后切换和删除。该记录不跨设备同步，不是服务端审计记录，也不构成无限长期记忆；券商账户同步不在当前范围内。
