...
- 依据 A18 赛题编写六章系统测试报告及证据附录，测试日期为 9 月 18 日；新增八幅 Mermaid 结果与机制图。
- 执行 15 项画像规则测试、29 项计算与存储检查、14 项真实 HTTP 检查，全部取得预期结果。证据保存在 `docs/submission/test-evidence/`。
- 单账户问卷读取负载为 100 并发任务、1,000 请求，全部返回 200，P95 为 6,018.757 毫秒。报告保留时延问题，完整咨询、100 名不同用户及长期可用性仍待验证。
- Word 共 28 页，正文 20 页、附录 6 页、封面目录 2 页，包含 8 幅图与 25 张表。OOXML 校验、证据一致性、Word 分页及 PDF 文字边界检查通过，独立子代理审核结论为 PASS。
- 按用户要求未进行视觉识别。未修改业务代码，未执行外部研究服务或证券交易。交付文件为 `docs/submission/Prism-系统测试报告.docx`，SHA-256 为 `d8de6d6088ad88de562a0b0769e3a53bfa249066cd07a7d8c48643791e45902d`。

## 2026-09-18：README 技术首页重写

- README 依据竞赛技术文档重新组织为项目介绍、系统方案、产品功能、核心技术、工程实现、本地运行、主要接口、系统边界和文档索引，删除阶段记录、完成率、并发观测和评测式章节。
- 复用九幅现有处理流程、系统架构、产品页面及核心技术图片；没有增加新的图片文件。
- Markdown 解析检查确认 14 个本地文档与图片目标均存在，差异格式检查通过。全量测试结果为 `905 passed, 9 skipped, 5 failed`；失败集中在行情测试数据和问财组合刷新路径，本次改动只涉及 README 与项目日志。

## 2026-09-18：流程图的层级与版式优化

- 参考 modern-web-design 的有限配色、留白和文字层级规范，重绘前端框架、后端框架与对话流程三幅图，统一灰白橙配色、图题、分区编号及图例。
- Mermaid 采用直角连线、并列模块、存储图形与入口强调，图源及 SVG、PNG 同步更新。Word 为三幅图预留更大展示高度，交付图形优化版并更新 README 入口。

## 2026-09-18：聊天工具流程与文档复核

- 多轮追问案例说明聊天调仓工具引导至计划入口，用户确认条件后执行确定性测算；持仓体检使用已确认的个人上下文。
- 图 5-8 展示请求入口、前提绑定处理、模型工具需求判定及一般查询、体检和调仓的分别处理流程。
- 第五章保留记忆架构、数据对象与上下文公式，持续校验及历史恢复的详细说明集中于第六章。文档结构、图表编号与二十二组公式转换检查通过。

## 2026-09-18：分层记忆架构与多轮咨询的形式化说明

- 第五章明确消息窗口、服务端事实和历史记录三部分架构，新增会话状态、上下文序列和 SHA-256 内容指纹公式，说明修订检查与保存方式。
- 第六章扩展输出期间连续校验、授权候选检索和历史恢复的形式化表达，增加会话前提状态表，以及条件变化和跨会话复用案例。六组新增公式均对应现有处理过程。
- 记忆存储、用户隔离和会话修订十七项测试通过；文档层级、工程路径、公式转换及图表编号检查通过。评审版保留十四幅图片，包含二十二组原生公式与十张表格。

## 2026-09-18：技术应用说明与对话记忆机制

- 第五章补充各组公式的应用入口、输入输出及实际演算；按参考文档调整开发环境说明，增加前端框架、后端框架和画像记忆数据流三幅 Mermaid 图。
- 说明最近六条消息、服务端会话事实与显式历史记录的保存和使用方式，补充多轮追问、条件变化、专业研究任务和非金融问题路由。
- 第六章围绕版本化事实约束、证据状态传递、交易后复核和历史记忆受控复用展开问题、处理步骤、案例及工程价值。同步更新工程核验索引。
- 记忆存储、用户隔离和会话版本十七项测试通过；公式转换、规则演算、正式报告复算及文档结构检查通过。Word 包含十四幅图片、十六组原生公式、九张表格与五篇参考文献。

## 2026-09-18：第五章与第六章的数学建模表达

- 第五章补充画像条件映射、来源等价类验证、穿透暴露矩阵、整数交易可行性、DAG 时间预算及双闸门状态聚合，使用十四组编号公式说明现有计算规则。
- 第六章围绕风险可行域、同源重复不变性、费用函数单调性和审查状态单调性组织四项创新，补充四组公式及其适用条件。文档正文改动限定于第五章和第六章。
- Word 保留十八组原生数学公式，公式编号单独排版。画像、证据和审查的 44 项测试通过，规则演算与正式报告复算通过。

## 2026-09-18：参照样文组织创新章节

- 阅读尚德重工、营养均衡、家和网络及智慧猫猫的创新正文，采用尚德重工的问题背景、核心流程、关键机制、工程价值层级，结合营养均衡的连续说明方式。
- 四项创新分别为 569、569、535、525 个中文字，总计 2,198 字；营养均衡的三项创新合计 2,182 字。每项说明业务问题、方法设计和使用价值，保留必要实例与文献引用。
- 更新交付入口为评审版 Word，生成配套 PDF，并核验章节、引用、图表及正式报告数值。

## 2026-09-18：创新章节的规则与演算

- 补充画像评分权重、分数分界、字段确认与版本检查；证据验证补充来源计数、冲突优先处理和五种输入演算；再平衡补充费用、交易单位、现金预留和实际程序输出；双闸门补充输入、检查条件、聚合规则及披露缺项案例。
- 直接调用项目程序核验 50 分至 40 分的画像变化、五种证据判定以及 400 股、5.04 元费用、5,994.96 元剩余现金的再平衡演算。画像、证据与闸门的 44 项单元测试通过，正式报告复算通过。
- 增加四张演算与判定表格。Word 生成器支持指定输出路径、步骤编号和短表格连续分页。原 Word 文件处于占用状态，交付文件另存为创新详解版，README 更新入口。

## 2026-09-18：参考格式与论文引用的 Word 技术文档

- 按用户参考资料重组八章正文，第五章为开发环境、前后端 AI 系统与核心技术，第六章按目标、流程、质量约束及价值说明四项创新。负责人、成员作用与成本相关章节按要求删除。
- 引入五篇 2025 至 2026 年原始研究，包含 2026 年 8 月发布的 LineageRAG 与 FrontierFinance，正文使用编号引用，文末提供作者、题名、年份和原始链接。算法描述核对画像预算、来源分组、再平衡和双闸门实现。
- 使用用户提供的 Word 样式生成可编辑 DOCX，保留 A4、楷体、分级编号及带页码目录；十一幅图片、三个原生数学公式、九张表格和五篇引用通过结构检查。真实保存报告的金额、比例及 HHI 复算通过。
- 使用本机 Word 更新目录并导出页面检查材料，检查产品截图、技术图、公式、图表分页和参考文献；README 提供 Word 与正文源文件入口。

## 2026-09-18：前端配色与 Python 技术图

- 应用 nature-figure 的信息层级与矢量输出方法，以前端主色 #e86f00、正文色 #202124 和灰白背景重绘六幅技术图。
- 使用三个阶段与并列模块呈现处理逻辑，橙色突出核心处理；详细依赖关系保留为可展开的 Mermaid。
- 绘图程序位于 tools/render_judge_figures.py，输出高清 PNG、可编辑 SVG 与字体嵌入 PDF。六幅图片完成检查，图中文字和模块文字边界检查通过。
- 文档结构检查与真实报告复算通过，覆盖九章、十一幅图片、八幅 Mermaid、十九张表格、三个公式和五十处工程路径。
- 完整技术文档导出为 Prism-技术文档.pdf，包含封面、目录、九章及工程附录，共 34 页；已检查全部页面缩略图、图表分页和文字提取。

## 2026-09-18：技术图统一视觉样式

- 采用 modern-web-design 技能中的字体层级、有限配色与留白原则，统一六幅技术图的图题、编号、副标题及阅读说明。
- 阶段区域使用浅灰底板，功能节点采用白底圆角卡片与类别色条，保留竖向阶段和并列分支；样式保存于 judge-diagrams.css。
- 六幅图完成渲染及图像检查，连线穿越无关节点检查为零处。文档结构、五十处工程路径及真实报告复算通过。

## 2026-09-18：技术图布局与连线

- 参考工程文档的研究编排及双闸门样图，重新组织六幅流程图：阶段竖向连接，并列关系在阶段内部横向展开，数据分支在所属区域汇合。
- 使用 Mermaid 与 ELK 生成六幅高清 PNG，正文固定展示图片，保留可展开的图示源文件；缩短跨区域连线并采用折线连接。
- 六幅图完成渲染与图像检查，几何采样未发现连线穿过无关节点。文档编号、链接、公式与真实报告复算通过。

## 2026-09-18：技术文档标题与图示表达

- 标题改为“Prism 个性化证券投顾智能体系统技术文档”，同步 README 入口名称。
- 参考旧版架构与个性化图的蓝、橙、绿、金色分区，调整六幅技术图的阶段分组、节点形状与状态分支；加入正式报告的总资产构成图和四项创新关系图。
- 八幅 Mermaid 图通过语法检查、浏览器渲染和图像检查；正文共十三幅编号图，包含五幅产品截图。公式、十九张表格、四十九处工程路径及真实报告数值复算通过。

## 2026-09-18：技术方案中的领域算法贡献

- 第五章补充画像驱动的风险约束映射、来源关联约束的证据交叉验证、交易单位与现金约束下的再平衡三项算法，明确输入、处理步骤及输出。
- 逐项核对预设评分权重与等级、来源分组与状态判定、数量取整与费用计算、可负担数量的二分查找及交易后复核。第八章将三项算法与应用价值、验证入口连接。
- 文档结构、公式、六幅 Mermaid 图、十九张表格与四十九处工程路径检查通过；真实保存报告的市值、总资产、占比及两项 HHI 复算通过。本次验证针对文档与案例。

## 2026-09-18：评审技术方案的功能展示与创新设计

- 第三章五项功能统一为需求说明、页面截图和三项能力标注，保持参考资料的功能展示顺序与编号图注。
- 第五章集中为画像参与计算、证据贯穿决策、金融结果可复算、专业协作与双重审查四项技术；数据状态归入第九章，组合计算功能与对应方法相邻。
- 第八章使用五列比较表说明四项创新设计、使用价值及核验入口，结合真实报告说明个性化风险判断与结果复核。
- 验证通过：九章正文、五幅截图、六幅 Mermaid 图、十九张表格、三个公式、七处行内数学、四十八处工程路径，以及第三、五、八章结构检查。真实保存报告的五项市值、总资产、占比和两项 HHI 复算一致；未进行视觉检查。

## 2026-09-17：面向评委的技术方案

- 参考指定共享对话及 FileRecv/1 四份技术方案的标题、目录、图注、功能展示与技术说明形式，更新 `docs/submission/competition-technical-solution.md`。
- 正文采用九章与工程附录，按需求、方案、功能、架构、核心技术、案例、验证、技术特色和工程保障组织；核心技术说明应用问题、处理方法、流程和结果。五幅现有截图与功能说明相邻，代码与接口索引集中于附录，技术图参考工程技术文档。
- 案例使用已保存正式报告，注明观察时间、采集时间和数值口径。五项证券市值合计 1,417,545.00 元、现金 28,000.00 元、总资产 1,445,545.00 元；主要占比及按报告舍入规则计算的两项 HHI 均复核一致，PortfolioReport 校验通过。
- 文档结构检查通过：五幅截图、七幅 Mermaid 图、十九张表格、三个公式、七处行内数学、目录跳转、图表编号与四十八处工程路径。使用 Marked、KaTeX、Mermaid 与 JSDOM 检查；未进行视觉检查。同步 README 文档入口，工程技术文档保留。

## 2026-09-15：整屏持仓截图 OCR 修复

- 三张原图复现：裁剪图成功，两张整屏图返回 EMPTY；OCR 文字完整，根因为双行配对固定 42 像素，而原图行距约 60 像素；图三还交换了盈亏、数量与价格的列位置。
- 采用表头几何位置定位列，按文字高度聚类和配对上下行；与自动裁剪后继续使用固定列方案相比，可同时适应分辨率和列顺序变化。限制在持仓表区域，零持仓单独保留，累计盈亏不冒充当日盈亏。
- 单独读取表格上方带标签的账户总资产与可用现金，保持未显示与零值的区别；汇总值参与确定性金额核对。
- 验证：三张原图通过隔离内存数据库的账户上传接口均返回 HTTP 200。图一/二为 6500 股、成本 11.580、现价 10.420；图三为 6700 股、成本 11.588、现价 10.030、现金 23933.53、总资产 91135.53。未确认或覆盖真实账户持仓，未提交原始截图。
- OCR 单元、账户隔离与 LIVE 价格来源回归共 17 passed；包含多种缩放比例和调整列顺序。截图报价仍保留截图来源与缺少完整时间的复核标记。

## 2026-09-15：发送左侧常驻进度

- 按用户反馈将进度移到发送按钮左侧，移除彩色标签背景、边框、圆点和步骤编号，使用低对比度文字与分隔符；当前步骤保留橙色强调，失败状态保留提示色。窄屏仅在进度区域换行，发送按钮保留右侧。
- JS 语法与 diff 检查通过，并在真实聊天页面核验位置及状态样式。

## 2026-09-15：聊天卡片、自动连接测试、报告与问卷

- 模型配置移除回复模式和独立连接测试按钮；保存到当前账户后自动测试，区分保存失败与已保存但连接失败。保留空 Key 不覆盖、失败输入不丢失及显式删除语义，重新打开仅标记已保存，不误称尚未测试。
- `query_stock_quote` 工具原声明支持名称，LIVE 底层却只接受代码。新增按官方证券检索精确匹配名称，歧义时要求代码；默认个股研判只调用行情与财务工具，避免无要求的研报/问财附加查询。
- 新增扶摇个股研究补充：行情与元信息之外调用估值快照、最近已披露季度报表和对应财务指标；严格匹配证券及报告期，保留空值、负估值、零值和独立失败原因。实际取得宁德时代 PE 17.22、PB 3.86、报告期加权 ROE 12.08%、毛利率 23.93%、资产负债率 63.65%（2026 中报）。估值快照时间不冒充财报披露时间，ROE 不自动年化。
- 自然语言工具结果与快捷查询复用同一下方个股卡片，聊天正文保留简短事实；移除每轮消息里的流程与工具标签，发送右侧常驻四步状态。卡片完整展示可用数据，不再因历史估值分位未接入而整体阻断，但未生成个人配置适当性分数。
- 正式报告以完整内容及规则版本生成稳定编号：相同快照后补行业、成本等内容生成新报告，原报告保留。真实页面加载和重复生成均成功，报告编号稳定且无全局错误。
- 问卷切换分组、上一部分和下一部分均滚动到题目顶部；Q12 使用影响程度，Q17 使用参考程度，1–5 的服务端评分不变。实际页面确认分组顶部约 20px，未改用户答案或确认问卷。
- 实际页面验收：保留已保存 Key 点击保存，自动测试 HTTP 200；输入“研判宁德时代”生成下方财务卡片；正式报告可见且重新生成无冲突；问卷程度选项及三种切页方式通过。历史估值分位与完整个股适当性仍无相应服务，未使用模拟值。
- 验证：最终全量 810 passed、9 skipped；相关定向 93 passed；Node 配置保存/自动测试成功与失败回归、JS 语法、diff 检查通过。全量初次两项失败为旧进度隐藏和旧字段文案断言，按新界面契约更新后通过。

## 2026-09-15：撤回人工行业确认，修正问财额度误报

- 用户要求行业自动识别。撤回 `5750d37` 新增的人工行业表单和 PATCH 接口；保留此前 Key 输入与持久化修复。原方案对问财行业能力的单源依赖在额度耗尽时阻断调仓。
- 自动行业采用东方财富公司资料 `RPT_F10_BASIC_ORGINFO` 的 `EM2016`，严格匹配完整 `SECUCODE`，再映射现有风险行业口径。2 秒请求超时、30 分钟成功缓存；未知行业不猜测。最初验证的 push2 行情资料随后发生连接中断，因此改用实际重复验证成功的公司资料接口。
- 保存行业及来源、获取时间，不改持仓价格、数量及行情时间；后续持仓保存保留服务端核验的行业。当前五只股票均真实查到行业，页面成功生成调仓测算，仍显示 HHI 3208.14 超过 2500 的风险复核，未执行交易。
- 更正前两条日志的诊断边界：问财财务、行业等 HTTP 401 正文明确写“您今天的次数已用完”，不能仅按 HTTP 状态认定凭据或权限问题。新增 `QUOTA_EXHAUSTED`，原始凭据错误仍保留 `AUTH_FAILED`；Agent 显示安全原因及 SkillHub 链接，无需重新填写 Key。
- 真实登录页面提交宁德时代 ROE、毛利率、资产负债率查询，模型调用 `query_financial_data`，页面准确展示额度耗尽与处理办法。外部额度尚未恢复，未声称已取得财务指标。
- 验证：全量 805 passed、9 skipped；最终公司资料接口及行业保留回归 42 passed；JS 语法与 diff 检查通过。服务继续运行于 127.0.0.1:8878。

## 2026-09-15：Key 表单、真实组合刷新与页面对话验收

- 回归定位：`360218a` 的状态渲染清空 Key 草稿，`e2da1dd` 将空 Key 保存解释为删除，`04a0f18` 的前端将扶摇报价与问财配置绑定。DPAPI 本身可跨重启保存；本次修正输入、删除语义及工作树默认存储路径。
- 模型设置：PUT 空 Key 保留同服务商已有配置，跨服务商必须重新填写；DELETE 显式恢复默认。异步加载不覆盖草稿，保存失败保留输入，连接测试显示具体安全错误，并阻止测试未保存配置与并发覆盖。
- 按需移植 `codex/repair-real-data-integration` 的批量真实报价、429 有界重试、问财中文行业解码、基金披露字段修复及已确认行业保留；保留主线 OCR 布局、正式报告和其他界面。旧探测失败不永久阻止配置完好的真实刷新请求。
- 价格与时间完整、仅股票行业缺失时允许部分体检，明确未分类占比与 HHI 的使用边界；不登记为可用于优化/调仓的完整刷新。新增 `/api/v1/advisor/portfolio/sectors` PATCH，仅对已保存股票确认行业，不改变价格数量；持仓明细提供对应入口，后续保存保留确认来源。
- 自然语言体检未锁定资料时增加确认入口；已绑定时展示确定性结果并说明采用锁定快照。调仓页面将泛化失败改为具体缺项与持仓入口。正式账户首页不再预加载不可用的 Fixture 场景目录。
- 实际页面验收在 `http://127.0.0.1:8878/` 完成：输入经模式切换后成功送达后端，重新打开后的空保存保留 Key/模型，真实连接与重启恢复通过；普通问答、个股行情与缺失财务原因、快捷体检、绑定资料后的自然语言体检均已验证。测试期间意外保存草稿后已告知用户，经用户明确同意使用本机已验证加密配置恢复当前账户，未输出密钥。
- 验证：全量 800 passed、9 skipped；最终定向 50 passed；Node 表单回归、JS 语法通过。全量初次两项失败分别是旧前端联动断言、旧单股探测 Mock；改为真实的新接口契约后通过。尚未替用户确认五项行业；问财财务等三项外部权限仍未解决，方案没有越过完整性闸门。原 8000 服务进程未替换。

## 2026-09-15：AI 对话回归、数据连接与本机配置兼容

- 提交追溯：`b9a406e` 引入正文工具校验，之前模型正文直接展示，不能作为真实数据能力已完整实现的证明；当前严格概念词匹配误拦截“请用通俗的语言解释一下市盈率”。另一个工作树 `codex/repair-real-data-integration` 含 `3d96231` 等未合入 main 的修复，不能将分支成果视为当前 main 已实现。
- 本机状态：已保存的 DeepSeek 配置可返回问答；扶摇真实行情成功但 ROE、毛利率、资产负债率为空。问财九项逐项实测公告、新闻、研报、宏观、基金、可转债 SUCCESS，行情、公司财务、行业 AUTH_FAILED；单独财务请求确认官方 HTTP 401。不能据此认定整枚 Key 失效，三项外部权限仍待核实。
- 修复概念问答的常见礼貌措辞；新增聊天 `query_financial_data`，复用问财既有六类结构化 ProviderOperation，保留原始字段、报告期及 SUCCESS/PARTIAL/EMPTY/FAILED。单项查询依据实际响应判断，不再被九项全部通过的汇总闸门阻断。结构化查询失败不撤销其他检索能力。
- 已锁定 LIVE 分析前提传入服务端完整风险画像，持仓体检调用现有 `calculate_portfolio_health`，显示行业预算、HHI、缺失项与核查状态；未锁定或 MOCK 前提不执行真实组合计算。调仓仍要求明确目标、换手约束及受信任刷新，未自动补造目标权重。
- 本机共享保护存储含另一工作树写入的 `verified_skills` 字段，当前 main 原先因额外字段校验导致应用导入失败；现兼容读取该元数据但不据此扩大就绪状态，保留原凭据。新增本机 `tools.wencai_setup` 只读逐项验证和隐藏输入更新，全部通过才替换凭据。
- 验证：新增回归初次 7 项失败，修复后通过。隔离真实行情环境变量的全量回归 782 passed、9 PostgreSQL skipped；最终配置兼容及对话定向回归 31 passed。首次全量的 1 项失败来自行情测试读取真实成交量而非固定测试值，移除该子进程环境变量后单项及全量通过，未修改用户环境或行情代码。实际保护存储下应用导入成功；隔离内存数据库的 HTTP/SSE + 真实模型概念问答和宏观查询均 HTTP 200、正文非空、无 error 且有 DONE；Agent 真实公告查询 SUCCESS，财务查询保留 AUTH_FAILED。
- 验收边界：未完成已登录浏览器的视觉验收；三项问财权限未修复；未合并其他工作树的整体变更，未替换独立研究矩阵或调仓服务。

## 2026-09-15：持仓结果展示第二批

- 持仓页四项 KPI 统一为组合市值、当日盈亏、累计盈亏、持仓数量；成本价或昨收价缺失时继续显示待补齐，不用默认值代替。
- 新增确定性正式快照报告：基于已确认持仓快照生成资产结构、收益摘要、集中度、浮亏、底仓保护、配置参考、画像区间（如已确认）、风险预算状态和行业明细；报告保存为 owner 与数据模式隔离的不可变快照，重复读取保持同一报告编号。
- 持仓明细增加诊断抽屉，展示数量、成本、现价、市值、组合占比和累计盈亏，并明确只做快照复核，不调用个股研究智能体、不输出买卖指令。
- 导出改为正式 HTML 报告，包含 KPI、摘要、资产结构、风险对照、持仓明细和使用说明；本地浏览器已实际下载并检查内容。
- 浏览器验证：桌面及 390px 窄屏均可见，无横向溢出，控制台无错误。相关报告、接口、存储与页面回归通过；当前未实现历史报告列表、智能体接入及个人资料/登录/多用户维护。

## 2026-09-15：问卷画像结果展示第一批

- 新增问卷快照到投资者结果页的确定性展示投影：八维 0–100 雷达图、画像标签、关键档案、服务策略、C1–C5 资产配置参考和风险提示；推荐智能体暂缓。
- 预览、确认和画像摘要接口统一返回该投影。画像展示只读取已确认 19 题问卷，不把交易记录或持仓快照作为展示前置条件；行为证据仍保留给既有内部风控链路。
- C4 资产配置按 PRD 图片示例固定为现金 10%、债券 20%、权益 70%，权益参考区间为 60%–80%；PRD未提供其余 C1/C2/C3/C5 的完整数值，当前使用版本化默认规则，后续可替换。
- 本地浏览器完成预览、确认、刷新后的结果展示验证；雷达、标签、策略、配置卡片均可见，控制台无错误，390px 下无水平溢出。相关问卷/API/UI 回归通过。
- 本批未实现持仓四项 KPI、个股诊断抽屉、正式报告导出链路、推荐智能体以及个人资料/登录/多用户维护。
...
## 2026-09-15 — 对话分流与 8000 正式报告实际验收

- 定位到旧 reload 子进程继续占用 8000，先前仅停止父进程未替换实际服务。核实并停止残留进程，以当前主工作区代码重新启动；未混入其他 worktree。
- 将已配置模型的对话改为结构化语义分流：普通问答直接生成；具体金融事实和个人持仓计算必须调用工具。未核验金融正文仍拦截，避免此前放行全部正文导致无依据价格输出。
- 真实 DeepSeek 请求暴露 thinking 与强制 tool_choice 不兼容的 HTTP 400；仅对此主机的强制工具请求禁用 thinking。分流增加一次模型请求。
- 撤销报告存储冲突被吞掉并返回假成功的逻辑；真实冲突返回明确错误码。实际报告生成、重复生成及按编号读取均成功。
- 在 8000 已登录页面实测原始问句、自我介绍追问、宁德时代研判和研判后的概念追问；研判卡片显示行情与财务数据。个人中心与持仓分析切换正常，最终代码重启后再次验证原句和报告。
- 纠正此前测试版本与提交版本不一致的验收结论；修复矛盾断言和旧 UI 文案断言。最终全量回归：827 passed、9 skipped、2 项依赖弃用警告；git diff --check 通过。跳过项不计为通过。
## 2026-09-17：持仓 OCR 无密钥证券代码补全

- 根因：原 OCR 仅在代码为空或格式非法时调用扶摇名称目录；其他电脑没有本机 DPAPI 凭据，名称补全直接跳过，而格式合法的 OCR 错码（如 `600000.SH`）又被误当成已解析身份。DPAPI 密文与 Windows 账户绑定，复制文件不能实现跨设备解密。
- 修复：新增上交所官方证券目录与深交所官方 A 股列表的无密钥精确简称查询。只在唯一精确匹配时绑定代码；两所返回冲突时失败关闭；单一交易所暂时不可用时允许另一所的唯一精确结果。合法格式错码与官方简称不一致时自动纠正，并保留 `SECURITY_CODE_CORRECTED` 人工复核标记。
- 安全边界：未把扶摇、问财或模型密钥写入仓库、浏览器或下载包。官方目录只用于证券身份，不将其标记为行情、财务或行业能力。跨设备共享真实数据仍需统一受控后端及明确的再分发授权。
- 验证：无密钥真实请求返回 `华天科技 → 002185.SZ`、`江苏新能 → 603693.SH`；新增与既有 OCR 聚焦测试共 `19 passed`，全量回归 `866 passed, 9 skipped`；Python 编译、前端 JavaScript 语法与差异格式检查通过。

## 2026-09-17：LIVE 能力首次探测失败后的自动恢复
...
## 2026-09-17：模型配置权限与默认模型修复

- 移除模型配置接口上的管理员角色限制，认证账户均可读取、保存、清除和测试安装级全局模型配置；运行时数据模式与问财配置的管理员限制保持不变。
- 模型默认值统一为 `deepseek-v4-flash`，覆盖服务端请求、LLM 环境变量缺省值、前端表单、服务商切换和静态页面快照。
- 验证：模型配置与本地访问相关测试 `75 passed`；Python 编译、JavaScript 语法和浏览器表单回归测试通过。

- 静态页面快照仅在 GitHub Pages 域名或显式 `?pages=1` 时接管请求，本地服务继续访问可写 API。

## 2026-09-17：文字录入持仓名称解析修复

- 根因：文字持仓解析器只遍历内置证券目录，未复用已有的官方证券简称目录；`华天科技`因此在数量与成本字段已识别的情况下仍返回 `EMPTY`。
- 修复：文字解析接入官方证券目录的唯一精确匹配，支持从数量附近提取证券名称，并明确识别 `买入成本`字段。LIVE 模式继续查询当前价格；当前模式无法取得当前价格时返回 `REVIEW_REQUIRED`，保留买入成本与当前价格的字段边界。
- 验证：`华天科技`解析为 `002185.SZ`；`100` 股、买入成本 `15.3`、当前价格 `16.97` 的回归通过；受影响测试 `112 passed`。
