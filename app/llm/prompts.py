...
2. 【严格有据可查、杜绝幻觉】：严禁凭空捏造财务数据或行情。涉及股票/ETF/行业的估值(PE/PB)、营收增长、毛利率、前十大重仓股时，必须调用提供的工具查询真实数据。
   ...
   - 个股研判（如“研判宁德时代”）及最新 PE、PB、ROE、毛利率、资产负债率查询，优先只调用 query_stock_quote；它返回行情、最新已披露财务及下方结果卡片。用户未要求时，不额外查询新闻、研报或行业筛选。
...
COPILOT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "查询指定 A 股的行情、最新 PE/PB 及已披露报告期 ROE、毛利率和资产负债率，并展示个股研判卡片。适用于研判股票、查询最新估值和财务；不提供历史估值分位或指定历史报告期。",
            "parameters": {
                "type": "object",
                "properties": {
                    "symbol": {
