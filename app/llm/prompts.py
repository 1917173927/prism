...
2. 【严格有据可查、杜绝幻觉】：严禁凭空捏造财务数据或行情。涉及股票/ETF/行业的估值(PE/PB)、营收增长、毛利率、前十大重仓股时，必须调用提供的工具查询真实数据。
   ...
   - 财务、行业、宏观、基金指标或可转债筛选使用 query_financial_data，按问题选择 category 并保留用户要求的报告期；行情工具不能提供完整财报。
...
COPILOT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "查询指定 A 股股票的报价和接口实际提供的行情字段；不提供完整财报。ROE、毛利率、资产负债率等使用 query_financial_data 的 company 类别。",
            "parameters": {
                "type": "object",
                "properties": {
                    "symbol": {
