...
function render(data) {
  ...
  graph.on('node:moved', () => status('位置已调整；请保存新版本'));
  graph.on('edge:dblclick', ({edge}) => { if (!busy) {graph.removeCell(edge); status('依赖已移除；请保存新版本');} });
}
...
async function save() {
  requireOwner();
  const definition = {...loaded.definition, nodes:graph.getNodes().map(node => ({
    node_id:node.id, ...node.position(),
