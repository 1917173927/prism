...
比赛要求的历史对话与连续追问在 Agent 首页形成独立验收链路。页面按 owner 保存多会话目录，支持新建、切换和删除；每个会话保留问题、流式回复和更新时间。请求端只选择同一上下文作用域中最近 6 条已经完成的消息，按用户与助手的原始顺序发送，当前问题不重复进入历史。一般知识问答使用 `general` 作用域；依赖已锁定组合前提的对话使用包含前提修订号的 `truth:workbench:<revision>` 作用域，从而在组合或前提变化后停止沿用旧对话。

| 验收能力 | 实现规则 | 确定性边界 | 页面证据 | 自动化检查 |
| --- | --- | --- | --- | --- |
| 历史记录 | 当前浏览器按 owner 保存最多 12 个会话，每个会话最多 40 条消息 | 不跨 owner、不跨设备，不写入决策审计表 | 左侧会话目录显示标题、轮次与时间 | 静态集成测试检查目录、切换、删除和存储命名空间 |
| 连续追问 | 同作用域最近 6 条完整消息随请求发送 | 当前问题不重复，未完成或失败消息不进入上下文 | 第二轮问题与回复保留在同一会话 | 流式行为测试核对第二轮请求的历史顺序与数量 |
...
历史记录属于交互连续性数据，不等同于第 10.4 节的结构化 Context Memory。Context Memory 保存经过确认且可审计的画像、组合和稳定引用；浏览器对话记录只帮助用户回看和形成追问。后端 `CopilotHistoryMessage` 将历史角色限制为 `user` 或 `assistant`、单条内容限制为 8000 字符、历史长度限制为 6 条，拒绝客户端注入 `system` 角色。模拟模式提供可重复的两轮输出，真实模型模式执行相同的历史边界和金融事实约束。
