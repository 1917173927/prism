...
系统根据数据用途组织短期消息、中期会话前提与长期历史记录。三类上下文分别承担语义连续性、当前事实约束及历史复用，保存方式如下。
...
会话前提从服务端当前记录生成，包含画像、持仓快照和数据模式。系统对规范化内容计算指纹，并为确认记录维护修订号。用户在分析过程中修改持仓或画像时，当前指纹与确认指纹出现差异，后续请求要求重新确认；流式输出期间也会再次核对，防止旧前提继续支撑当前结论。

长期记忆保存经过校验的结构化对象及内容哈希。检索按用户归属读取最近 100 条显式记录，配置模型时根据风险等级、期限、资产名称及意图等描述字段排序；未配置或模型不可用时返回带原因标识的有限关键词匹配。检索结果保持 HISTORICAL_ONLY 状态，不自动覆盖当前画像与持仓。用户选择恢复后，界面载入历史资料并清空派生结果，当前分析还需重新确认与计算。

#### 5.3.8 多轮追问与工具需求路由

每轮问题均重新判定是否依赖外部金融事实或个人持仓计算。分类器结合当前问题及近期消息，返回明确的工具需求标记。一般闲聊、自我介绍、写作翻译、概念或公式解释允许进入一般回答；具体证券行情、财务新闻、个人持仓分析以及金额和调仓测算要求调用工具。无法确定时按需要工具处理。

例如，用户先询问某只股票的持仓风险，随后追问“如果减持一部分，现金会增加多少”，系统借助近期消息理解指代，同时从服务端确认前提读取数量与画像，并调用确定性测算。用户提出“为什么这个比例超限”时，回答可结合已有计算依据解释预算规则；如果追问改变了持仓或投资条件，则需要更新资料并重新确认相关前提。
...
图 5-8 画像记忆对话与专业分析的数据流

在图 5-8 中，结构化建议路径经过独立双闸门，聊天工具转述路径根据工具结果组织回答。二者共用用户资料和事实来源，按各自入口执行结果检查。历史检索结果需要经过用户选择与确认，才能重新成为当前分析输入。
