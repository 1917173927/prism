...
常见画像系统止于“保守型、均衡型、成长型”的结果展示。Prism 将已确认画像继续转换为风险预算和配置边界，使画像直接参与单一资产、行业、科技行业和未分类暴露判断，并影响目标结构与再平衡复核。

### 二、 证据型多智能体协作

Prism 不使用简单多数投票替代金融判断。中央协调器通过 DAG 管理任务依赖，专业节点返回结构化证据、事实、发现和状态；结果聚合检查引用与来源独立性。节点数量不会自动提高结论可信度，只有证据闭合才能升级结论。

### 三、 大模型与金融计算物理隔离

LLM 负责理解用户要分析的问题，确定性引擎负责计算金额、比例和阈值。两者通过结构化契约连接，模型不能直接输出权重守恒、基金穿透或调仓股数。该边界提升了可复算性、模型可替换性和合规可控性。

### 四、 数据质量状态全链路传播

系统严格区分 SUCCESS、PARTIAL、EMPTY 和 FAILED。缺失数据不填零，失败不伪装为空结果，缓存不伪装为实时数据。数据状态沿研究、计算和审查流程传播，使用户能够看到结论受到哪些条件限制。

### 五、 双闸门与可验证决策回执

风险闸门和合规闸门独立执行，任何一侧未通过都不能生成正式建议。通过后的 DecisionReceipt 绑定画像版本、持仓快照、研究结果、证据引用、闸门状态和规则版本，能够验证一条建议在何种条件下产生。

### 六、 从结论到来源的双向解释

工作台同时展示结论卡、风险阈值、研究流程和证据详情。用户既可以从组合风险下钻到具体资产，也可以从 Finding 反向查看 Fact 与 Evidence；历史回执保留对应时点的输入与结果。

表 6-1 创新机制与项目价值

| 创新点 | 传统做法的不足 | Prism 核心机制 | 可观察成果 |
| --- | --- | --- | --- |
| 画像约束闭环 | 画像只作标签或推荐分类 | 画像—预算—暴露—调整连续计算 | 个性化阈值与组合风险并列展示 |
| 证据型多智能体 | 节点输出文本后直接汇总 | DAG、结构化输出、血缘与交叉验证 | 节点状态和证据链可展开 |
| LLM 与算法隔离 | 模型直接生成金融数值 | 语言理解与 Decimal 计算分层 | 公式、分母、精度可复算 |
| 质量状态传播 | 缺失、空值和失败混用 | Provider 四态与 stale 标记 | 结果明确降级而非伪成功 |
| 双重合规裁决 | 生成后追加免责声明 | 建议生成前执行风险与合规硬闸门 | PASS、REVIEW_REQUIRED、BLOCKED |
| 可验证回执 | 历史只保留一段文本 | 输入版本、证据和裁决共同留痕 | DecisionReceipt 与 DecisionEvent |
