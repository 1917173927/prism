...
算法接收已确认画像和组合暴露，依次执行评分映射、预算选择、同口径比较及结果记录。画像版本与预算共同保留，输出包含具体资产或行业、观察值、上限和超限幅度。用户更新画像后，后续计算依据新的确认版本执行。

#### 6.1.3 质量约束与验证

同一组合在不同画像下使用不同预算规则。第七章报告中最大资产占比为 52.83%，对应成长型的 50% 上限，差额为 2.83 个百分点。将该观察值代入保守型 20% 上限时，算术差额为 32.83 个百分点。后者用于说明规则敏感性，正式报告采用成长型画像。

#### 6.1.4 工程价值

该算法把个人条件连接到实际计算，并提供字段、版本、规则和结果之间的追踪关系。它是面向投顾场景实现的规则映射与约束检查方法，评分权重和风险上限均可在工程定义中核验。
...
算法以主体、指标、单位和期间确定声明范围，排除不满足条件的观测；随后按来源关联标识分组，统计独立支持与反对来源。重复来源、组内冲突和缺少关联均形成明确问题记录。有效事实沿引用关系进入研究发现及建议审查。

#### 6.2.3 质量约束与验证

算法按来源组计算独立支持数量。同一来源出现多次时，只增加对应证据记录；同组存在不同数值时，进入冲突处理。这样可以检查“资料数量增加”是否同时带来了独立依据。相关实现位于 app/research/cross_validation.py。
...
算法先由权重差计算目标金额，再通过报价及交易单位取得候选数量。卖出更新现金，买入在整数交易单位范围内执行二分查找，每次判断都计入费用与现金预留。最后汇总资金流并对交易后组合复核。

#### 6.3.3 质量约束与验证

程序核对卖出数量不超过持仓、买入资金与费用不超过允许支出，以及现金汇总与行动记录一致。二分查找用于确定当前候选行动的可负担数量；多项行动执行顺序和组合级风险仍由整体复核检查。实现位于 app/service/portfolio_rebalancing.py。

#### 6.3.4 工程价值

该方法将金融建议转化为可复算的测算步骤，明确显示目标结构、测算数量、费用和调整后的风险。用户可以查看执行条件对方案的具体影响，结果保留为方案测算。
...
研究节点按照依赖执行，引用随结果传递，缺项和超时进入状态汇集。风险闸门与合规闸门分别运行，再按明确的状态优先级聚合。每次判定保留研究、画像、组合及审查记录，支持后续复核。

#### 6.4.3 质量约束与验证

风险检查通过时，合规检查仍须独立通过；研究任务完成后，数据与引用仍须满足使用条件。状态聚合规则由 app/gates/pipeline.py 定义，结果进入建议组合器的再次校验。

#### 6.4.4 工程价值

系统将“完成研究”和“能够提供完整建议”分别建模，说明每个结论的使用条件。任务依赖与双重审查共同支持问题定位，能够明确指出需要补充的资料和受影响的分析结果。
