...
Prism 将投资者画像、投资组合、专业研究、金融计算、风险审查和决策解释组织为一条可展示、可验证、可追踪的服务链路。系统从投资者画像和持仓上下文出发，完成专业研究、确定性计算、风险审查和决策解释，输出带有来源、时间、状态与计算依据的结果。

当前可访问演示：[Prism Pages 演示](https://prism.daoyezongzi.org/?pages=1)。公开页面通过静态快照回放本地服务捕获的接口响应，页面保留原始数据模式和结果状态；当前代码、运行条件和验证依据见第 7、9、10 章。
...
| 可解释 | 展示来源、事实、发现、计算过程、风险条件和失效条件 | 结论形成过程清晰 |
...
* 对结果复核，系统为每项结论提供来源、时间、状态、关联标识和决策事件，支持从最终结果回看处理过程。
...
图 1展示从用户上下文到研究、计算、审查和决策回执的完整关系。
...
当前界面以 `app/api/static/index.html` 和 `app/api/static/app.js` 的现行页面结构为准。Agent 任务中心承担统一入口，组合、研究、解释和评测页面按任务展开。
...
可直接打开 [Prism Pages 演示](https://prism.daoyezongzi.org/?pages=1) 查看当前页面。页面以“任务入口—分析结果—账户条件”的布局组织内容，主要区域如下：
...
系统将研究节点、证据链和决策解释集中到研究工作台与审计页面，可以从研究主题进入节点结果，再进入来源和计算过程。
...
系统将数据服务状态、证据质量状态和建议审查状态分别表示，页面和接口结果能够区分处理进度与结果性质。
...
运行状态页面展示当前数据模式、提供方能力、查询状态和结构化问题。相关测试覆盖提供方接口规则、恢复处理、运行模式和固定数据状态。

### 5.7 数据真实性与缺失语义

金融数据进入研究或组合计算前，需要同时保留内容状态、时间口径和来源关系。数据提供方状态、送达方式和证据质量分别记录，缓存或备用来源不会改写原始内容状态。

| 检查维度 | 必须保留的内容 | 处理规则 | 核验对象 |
| --- | --- | --- | --- |
...
这些页面和验证路径覆盖账户、组合、市场和画像等主要结果，具体运行状态见第 9.2 节。
...
Prism 将“回答问题”提升为“组织可验证的决策材料”。研究结论、组合计算和建议回执共享来源关系，使用者可以沿着证据链回看处理过程。
...
系统把复杂处理过程转换为工作台卡片、研究矩阵、证据浏览、因果解释、情景对比和再平衡步骤。产品界面与技术对象相互对应，使用体验和工程实现可以相互验证。
...
| 依据充分 | 代码、测试、固定案例和运行工具形成验证依据 | 支撑实现可信度 |
...
应用通过 `create_app()` 生成 FastAPI 应用，静态工作台由应用统一提供，领域服务通过依赖注入连接存储和数据提供方。SQLite 适合本地演示与验证，PostgreSQL 适配用于需要独立数据库服务的运行环境。
...
数据结果携带来源、观察时间、获取时间、期间、单位、缺失字段和状态。页面能够区分完整结果、部分结果、明确空结果和执行问题，保证每项数据都有清晰的来源与状态。
...
| 类型 | 内容 |
| --- | --- |
| 技术设计 | `docs/submission/technical-report.md` |
| 服务入口 | `app/api/main.py`、`app/api/static/index.html`、`app/api/static/app.js` |
| 页面与快照 | `app/api/static/pages-snapshot.js`、`app/api/static/pages-snapshot.json`、`tests/browser/test_pages_snapshot.mjs` |
| 领域模块 | `app/profile/`、`app/research/`、`app/portfolio/`、`app/risk/`、`app/recommendation/` |
| 技术图示 | `docs/submission/figures/` |
| 验证工具 | `tests/`、`tools/evaluate_mvp.py`、`tools/load_test.py`、`tools/http_load_test.py` |

### 10.2 关键架构决策

| 决策 | 实现方式 | 形成的能力 | 主要实现位置 |
| --- | --- | --- | --- |
| 模块化单体 | 接口、应用服务、领域目录、数据提供方和存储在同一进程协作 | 模块职责清晰，便于本地运行和完整验证 | `app/api/main.py`、`docs/adr/0001-modular-monolith.md` |
| 结构化研究有向无环图 | `ResearchPlan`、节点状态、依赖门控和有界执行器协同工作 | 研究任务具有顺序、状态和故障位置 | `app/orchestration/`、`app/service/specialist_matrix.py` |
| 证据优先 | `Evidence → Fact → Finding → Recommendation` 贯穿研究和回执 | 研究发现能够回到来源和验证状态 | `app/contracts/evidence.py`、`app/research/` |
| 确定性金融计算 | 画像评分、暴露、穿透、集中度、风险预算、优化和再平衡由 Python 服务计算 | 金额、权重和风险结果可以复核 | `app/profile/`、`app/portfolio/`、`app/risk/`、`app/optimization/` |
| 数据状态分层 | `SUCCESS`、`PARTIAL`、`EMPTY`、`FAILED` 与来源、指纹和问题对象绑定 | 空结果、部分数据和执行问题具有明确含义 | `app/providers/contracts.py`、`app/providers/` |
| 双闸门审查 | 风险闸门、合规闸门和建议组合器分别检查输入、引用和披露 | 建议资格由明确条件裁决 | `app/gates/`、`app/recommendation/` |
| 用户归属与内容哈希 | 账户绑定 `owner_id`，开发模式使用 `X-Owner-ID` 隔离对象，事件保存内容哈希 | 结果具有用户、版本和内容身份 | `app/api/access.py`、`app/store/`、`app/gates/fingerprint.py` |
| 固定数据与真实数据分层 | `MOCK` 读取固定数据，`LIVE` 按能力探测和正式数据规则运行 | 演示可重复，真实数据状态单独表达 | `app/runtime/`、`app/providers/runtime.py`、`app/api/static/` |
| Pages 快照发布 | `pages-snapshot.js` 回放 `pages-snapshot.json` 中保存的接口响应 | 公开页面能够展示可重复的账户结果 | `app/api/static/pages-snapshot.js`、`app/api/static/pages-snapshot.json` |

### 10.3 运行条件与能力状态

| 运行维度 | 系统能力 | 运行条件 | 当前状态 |
| --- | --- | --- | --- |
| 数据模式 | `MOCK` 固定数据、`LIVE` 真实提供方能力和操作级状态 | `LIVE` 需要凭据、能力探测和正式接口规则验证 | `/api/v1/runtime/data-mode` 和能力矩阵提供状态 |
| 投资者输入 | 19 道问卷、画像提案、行为画像、持仓导入、图片识别和基金成分快照 | 输入需要通过归属、版本、时间、字段和一致性检查 | 快照、草稿、确认状态和问题码 |
| 研究服务 | 市场、行业、股票、基金、ETF、可转债研究、研究流水线和证据链 | 固定服务用于回放，实时路径按能力探测和数据状态处理 | 节点、运行、证据和发现状态 |
| 组合分析 | 暴露、穿透、集中度、风险预算、配置范围、目标结构、情景和再平衡 | 持仓、报价、行业、现金和成分数据满足接口规则 | `READY`、`REVIEW_REQUIRED`、`BLOCKED` |
| 对话与模型 | 意图识别、工具调用、画像提案、流式回复和解释 | 远程模型需要配置，金融事实通过结构化服务获得 | 模型配置、工具结果和流式事件 |
| 持久化 | SQLite 默认存储、可选 PostgreSQL、决策事件、记忆、账户和审计 | 数据目录、数据库连接、迁移和权限可用 | 版本、内容哈希、迁移记录和问题码 |
| 交易支持方式 | 再平衡金额、数量、费用、现金和交易后风险测算 | 输入有效并能够查看行动计划 | `ADVISORY_ONLY` |
| 部署方式 | 本地回环服务、同源静态页面和真实 HTTP 只读观测 | 启动脚本、运行依赖、凭据和上游额度可用 | 健康状态、探测结果和提供方状态 |

当前本地应用默认访问 `127.0.0.1:8000`，使用 `data/private` 保存数据库。Windows 默认使用 `ProtectedSecretStore` 保存受保护凭据。问财、扶摇、雅虎财经、港股资讯网和同花顺量化接口按照各自配置与协议提供数据；外部服务额度、数据留存、展示和再分发授权由部署环境单独确认。

### 10.4 当前限制

| 能力 | 当前状态 | 对系统使用的影响 | 增强方向 |
| --- | --- | --- | --- |
| 问财部分技能额度 | `hithink-market-query` 当前曾返回 401，运行状态记录 `QUOTA_EXHAUSTED` | 依赖该技能的实时字段可能进入复核或失败状态 | 额度恢复后重新执行能力探测和财务复验 |
