...
问卷包含八个展示维度；基础风险评分使用损失承受、投资期限、流动性需求、投资经验和收益预期五个离散维度。风险等级进入版本化预算规则，决定表 5-1 中的适用上限，计算使用所属报告的统一分母。
...
验证同时检查字段、时间、期间与来源独立性。同源数据的不同呈现形式不能增加独立证据数量；字段缺失、来源冲突和过期内容保留质量状态。研究结果与历史回执据此支持逐层复核，完整建议生成还需通过双重审查。数据服务的四类结果状态见第九章。
...
再平衡使用报价、交易单位、费用和现金计算数量与行动顺序，随后复核交易后的现金与风险条件。证券市值与现金之和必须等于总资产；情景结果对应用户给定的冲击条件。
...
| --- | --- | --- | --- | --- |
| 个性化 | 在提示词中描述投资者类型，由模型调整回答 | **画像参与计算**：确认画像直接决定风险预算与配置范围 | 同一组合可按不同画像得到不同超限判断 | 表 5-1 风险预算、第六章实际报告 |
| 研究依据 | 检索增强生成（RAG）为回答附上相关资料 | **证据贯穿决策**：Evidence → Fact → Finding → Recommendation 逐层引用并校验 | 从建议回到事实与来源，检查缺项、冲突和失效条件 | 图 5-2 证据流程、附录证据与建议模块 |
| 金融计算 | 由 LLM 在生成回答时推导金融数值 | **金融结果可复算**：独立程序计算金额、权重与风险，模型解释结果 | 可用同一输入和明确规则复核计算结果 | 第六章案例、表 7-2 数值复核 |
...
Prism 在完整建议链路中同时保存资料来源、事实、发现与审查结果。**可信度具有逐项可检查的条件。** 某项结论依赖的数据发生变化、存在来源冲突或引用缺失时，可以沿关联关系定位受影响的分析依据，并按规则进入复核或阻断状态。
