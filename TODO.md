...
- [x] Windows 本地且启用账户身份绑定时，个人模型密钥改为 DPAPI 保护的 owner-scoped 持久化：填写一次后可跨服务重启恢复，API 不回读明文，留空保存删除个人配置；未认证开发模式仅使用进程内配置，非 Windows 部署继续要求外部 Secret/环境变量。
...
- [ ] 完整个股研究仍缺真实财务基本面字段；问财/SkillHub 未配置，宏观、行业、公告、语义检索及组合刷新不能验收；默认 Fixture 研究矩阵、专题研究、组合优化、情景模拟和固定工作流尚未替换为 LIVE service adapter。
