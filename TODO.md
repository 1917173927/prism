...
- [ ] 核实当前问财行情、公司财务、行业 Skill 的 HTTP 401；其余六项逐项实测成功。运行 `python -m tools.wencai_setup` 可重新验证，需更新 Key 时使用 `--configure`。当前聊天不再因三项失败阻断其他可用查询。
...
- [x] 移植真实组合批量报价与行业解码修复；报价完整而股票行业缺失时返回部分体检，方案显示缺项并提供行业确认入口。完整性不足的组合仍禁止优化与调仓。
- [ ] 当前账户五只股票行业尚未确认。可在持仓明细核对后逐项确认；未替用户推测分类。问财行业/财务/行情权限修复后还需重新验收完整数据链路。
